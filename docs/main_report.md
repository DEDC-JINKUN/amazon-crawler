...
> 更新日期：2026-09-04（develop候选，未发布）
> 当前生产对象：Amazon.com；自有 ASIN 1,093 条
> 当前状态：本分支基于正式main `2e30368`。恢复表生产迁移已执行；本轮在独立develop工作树开发美国站可选ZIP与简明控制台，不改变正式配置、不停止既有采集、不新增真实采集批次。未合入main，不能宣称已部署或真实采集性能达标。
>
> 本轮待发布变更：[美国站采集与简明控制台](develop_us_marketplace_20260904.md)。ZIP留空为美国站观察，填写才要求指定地区；正式配置的90001仍未改动，发布时另行切换。下文旧固定ZIP运行结论保留为历史，不改写raw/evidence。
>
