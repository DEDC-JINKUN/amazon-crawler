...
> 更新日期：2026-09-04（代码集成与部署预检，尚未执行生产恢复表迁移）
> 当前生产对象：Amazon.com；自有 ASIN 1,093 条
> 当前状态：正式代码已快进至批准候选 `3524076`；用户方案结构保留。恢复表生产迁移、重启及新固定20验收尚未完成，不能宣称生产失败已解决。
>
> 合并依据：共同基线 `c3af4b2`、用户方案原文和候选 `3524076`。本次只更新部署状态与已批准合同；旧运行结论保留为历史，不改写 raw/evidence。完整原文和补丁的可核验归档见文末。
