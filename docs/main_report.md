...
### 10.4.1 流量和“200 元千万级”验证口径

“千万级数据”必须先明确是页面数、ASIN 快照数、标准化表记录数还是非空字段值数；它们不是同一个成本单位。当前可复现批次中，10 个 HTTP 2xx 页面中只有 9 个 ASIN 通过商品页校验；去重后本地响应体为 15,543,610 bytes，平均 1.647 MiB/成功 ASIN，5,800 个商品页线性外推约 9.33 GiB。这是未压缩响应体基线，不是代理商最终账单。

生产验证必须使用同一批 ASIN 分别运行 gzip 和 identity，以 evidence 的 `transfer_bytes` 和代理后台用量/费用前后差值作为真实口径。报告至少应包含：成功 ASIN 和成功率、HTTP/评论/Firefox 请求数、计费 bytes、元/成功 ASIN、重试和 403/429/CAPTCHA 比例。只有在“一条数据”定义、代理账单和成功率都确定后，才能判断 200 元是否达标。
