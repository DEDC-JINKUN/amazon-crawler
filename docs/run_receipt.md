...
`run_scheduled_windows.bat` 和 `run_once_windows.bat` 会在每次采集后自动写入 `data/amazon_us/run_receipt.json`。回执使用临时文件替换，写入中断不会截断上一份回执；读库失败时不覆盖旧回执。回执生成失败不会覆盖已有验收文件；脚本仍会先返回 worker 或验收的原始错误码。
