...
- [x] README 交接入口补充 evidence/metrics 检查命令。
- [x] 竞品候选发现→运营批准→正式 manifest 端到端离线集成测试。
...
- 美国 VPN + ZIP `90001` 的 10 条商品批次：10 个 HTTP 2xx 页面中 9 个 ASIN 通过商品页校验，1 条 ASIN 不一致。原始 evidence 体积为 17.60 MB；按重用 `raw_html_path` 去重后为 15.54 MB，成本外推分母只用 9 个成功 ASIN；
- 30 条商品批次：第 1 个 ASIN 遇 CAPTCHA 后按设计停止，不能据此承诺 30 条吞吐；
- 离线解析：约 2.64 页/秒，说明当前主要瓶颈是网络、评论分页和访问阻断；
- 当前默认仍只跑一个 ZIP；多区域仅用于区域库存/配送分析；
- 公司数据库、付费代理池、云对象存储和 Weknora 尚未接入。

下一阶段门槛：先用批准的出口完成 10 条稳定回归和评论分页验证，再扩大到 30 条；每次必须保存 run metrics、evidence 健康报告和失败 ASIN 清单。
