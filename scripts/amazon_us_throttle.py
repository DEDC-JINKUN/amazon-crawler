...
class EgressLimiter:
    ...
    jitter_fraction>0 时在令牌间隔上追加 0~fraction 的均匀随机延迟，
    打破固定速率的机器统计特征（类人类节奏，主档 ±15% 抖动的安全半边）。
    """

    def __init__(
        self,
        global_rate_per_second: float = 0.0,
        egress_rate_per_second: float = 0.0,
        burst: int = 1,
        ...
        *,
        ...
        self.global_bucket = TokenBucket(global_rate_per_second, burst, clock=clock, sleeper=sleeper)
        self.egress_rate = float(egress_rate_per_second)
        self.burst = int(burst)
        self.jitter_fraction = max(0.0, min(float(jitter_fraction), 1.0))
        self._clock = clock
    ...
    def _egress_bucket(self, egress_id: str) -> TokenBucket:
        with self._lock:
            return self._buckets.setdefault(egress_id, TokenBucket(self.egress_rate, self.burst, clock=self._clock, sleeper=self._sleeper))
    ...
    def acquire(self, egress_id: str = "direct", timeout: float | None = None) -> float:
        ...
        if self.jitter_fraction > 0:
            reference_rate = max(self.global_bucket.rate, self.egress_rate)
            if reference_rate > 0:
                extra = (1.0 / reference_rate) * self._rng.uniform(0.0, self.jitter_fraction)
                if extra > 0:
                    self.sleeper(extra)
                    waited += extra
        return waited
