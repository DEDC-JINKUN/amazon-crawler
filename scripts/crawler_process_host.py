...
def parse_owner_start_time(raw: object) -> datetime:
    """解析 owner 启动时间为 UTC datetime。

    .NET 的 'o' 往返格式带 7 位小数秒（tick 精度），
    而 Python 3.8~3.10 的 fromisoformat 只认 3 或 6 位小数秒，
    所以先把小数秒截断/补齐到 6 位（微秒精度，
    截断误差 < 0.1 微秒，远小于进程身份校验的 100 tick 容差）。
    """
    text = str(raw).strip().replace("Z", "+00:00")
    match = re.match(r"^(.*T\d{2}:\d{2}:\d{2})\.(\d+)(.*)$", text)
    if match:
        text = f"{match.group(1)}.{(match.group(2) + '000000')[:6]}{match.group(3)}"
...
def open_owner_monitor(
    ...
    try:
        expected = parse_owner_start_time(owner_start_time)
