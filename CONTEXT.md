...
**美国站采集**:
采集美国站、美元计价的目标商品，配送地区随当次页面记录，不承诺某个指定ZIP的价格与库存。
_Avoid_: 全美统一价格、固定90001

**指定ZIP采集**:
任务明确要求某个配送ZIP；未确认该配送地区时，价格、库存与配送信息不得视为目标地区的结果。
_Avoid_: 美国IP即已确认配送地区

**待重试**:
尚未取得合格结果、仍有恢复额度的商品；验证码、网络超时等是其具体原因，而不是最终成功结果。
_Avoid_: 所有blocked都是失败、遇过验证码就是未完成
