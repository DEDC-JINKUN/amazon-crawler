...
def test_http_adapter_bridges_only_confirmed_isolated_firefox_cookies_into_current_run():
    ...
    with patch.object(worker.urllib.request, "build_opener"), patch.object(
        worker.RunScopedAmazonCookieSession, "setup_us_delivery_context", return_value=True
    ):
...
def test_selenium_447_deferred_registry_leaks_stale_fail_request_after_user_callback():
    # RequestHandlerRegistry 是 selenium 4.28+ 才有的内部模块；
    # 本机 Python 3.8 只能装到 4.27.1（4.28 起不再支持 3.8），没有该模块时跳过
    pytest.importorskip(
        "selenium.webdriver.common.bidi._network_handlers",
        reason="selenium<4.28 没有 RequestHandlerRegistry（Python 3.8 上限 4.27.1）",
    )
    from selenium.common.exceptions import WebDriverException
...
def test_product_fallback_reason_and_nullable_browser_traffic_are_persisted_in_evidence():
    ...
    class Adapter:
        ...
        def fetch(self, url):
            # HTTP 返回缺少 title 的 HTML（硬错误），触发 core fallback 到 browser
            return """
    ...
    # 硬错误（缺少 title）触发了 browser fallback，而不是软错误 context_mismatch
    assert adapter.browser_calls == [("missing_title", "run-1", "B00RCPDCQU")]
    ...
    assert context["fallback_reason"] == "missing_title"
    assert context["fallback_reasons"] == ["missing_title"]
...
def test_postgres_postal_only_mismatch_saves_partial_product_without_cookie_bridge():
    ...
    # postal mismatch 不再触发 browser fallback（只有硬错误才触发）
...
# GEO mismatch（货币/配送地址）不再是硬错误——HTTP 路径自动设美国位置 cookie
# 真正的硬错误只有 ASIN 身份不匹配（变体跳转、抓错商品）
@pytest.mark.parametrize(
    ("price", "delivery", "_ignored"),
    [
        ("HKD117.52", "Delivering to Portland 97230", None),
        ("$19.99", "Delivering to Hong Kong", None),
        ("", "Delivering to Portland 97230", None),
        ("$19.99", "", None),
    ],
)
def test_postgres_geo_mismatch_no_longer_triggers_context_mismatch(price, delivery, _ignored):
    worker = load_worker()
    ...
    payload = storage.saved[0]
    # GEO mismatch 不再触发 context_mismatch / browser fallback
    assert adapter.browser_calls == 0
    assert adapter.commit_calls == 0
...
def test_sqlite_missing_us_evidence_remains_failed_after_firefox_is_unavailable():
    ...
    assert adapter.browser_calls == 0
    assert adapter.commit_calls == 0
    # GEO mismatch 不再导致 failed → 产品直接 succeeded
    assert state["status"] == "succeeded"
    # product_snapshot 应该有记录了（因为 succeeded）
    assert product is not None
    # context_quality 不再是 invalid
