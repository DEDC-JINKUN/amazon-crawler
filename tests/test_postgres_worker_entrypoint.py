...
class LoginWallReviewAdapter(ReviewAdapter):
    """评论页返回登录墙：亚马逊限制未登录浏览评论（常态）。"""

    def fetch(self, url):
        return "<html><head><title>Amazon Sign-In</title></head><body>Sign in</body></html>", 200
...
class ReviewLinkAdapter(Adapter):
    """商品页带评论链接（正常情况下会进评论翻页阶段）。"""

    def fetch(self, url):
        return """
        <html><head><link rel="canonical" href="https://www.amazon.com/dp/B00RCPDCQU"></head><body>
          <input id="ASIN" value="B00RCPDCQU"><span id="productTitle">Example</span>
          <a data-hook="see-all-reviews-link-foot" href="/product-reviews/B00RCPDCQU">See more reviews</a>
        </body></html>
        """, 200


class ReviewOffStorage(Storage):
    """领取的任务带评论开关=false（批次建批时选了不采评论）。"""

    def claim_task(self, worker_id, lease_seconds=None):
        task = super().claim_task(worker_id, lease_seconds)
        if task is not None:
            task["collect_reviews"] = False
        return task


def test_review_toggle_off_finishes_product_without_review_stage():
    # 评论开关关：页面有评论链接也不进评论阶段——商品完成即收尾，
    # 评论汇总标 skipped，next_status=succeeded
    worker = load_worker()
    storage = ReviewOffStorage()
    config = dict(worker.DEFAULTS)
    config.update({"max_actions_per_run": 1, "raw_html_dir": None, "context": {}})

    assert worker.run_postgres_actions(storage, ReviewLinkAdapter(), config, limit=1, worker_id="worker-a") == 1
    payload = storage.saved[0]
    assert payload["next_status"] == "succeeded"
    assert payload["reason"] == "reviews_skipped"
    assert payload["review_summary"]["status"] == "skipped"
    assert payload["review_summary"]["next_page"] is None
    assert payload["state_fields"]["next_review_url"] is None


def test_review_toggle_on_still_queues_review_stage():
    # 评论开关开（默认）：页面有评论链接 → 进评论阶段 reviews_pending
    worker = load_worker()
    storage = Storage()
    config = dict(worker.DEFAULTS)
    config.update({"max_actions_per_run": 1, "raw_html_dir": None, "context": {}})
    ...
    assert worker.run_postgres_actions(storage, ReviewLinkAdapter(), config, limit=1, worker_id="worker-a") == 1
    payload = storage.saved[0]
    assert payload["next_status"] == "reviews_pending"
    assert payload["reason"] == "reviews_required"
    assert payload["state_fields"]["next_review_url"].endswith("/product-reviews/B00RCPDCQU")
...
def test_postgres_runner_persists_explicit_sibling_identity_without_product_data():
    ...
    # 页面是兄弟 ASIN（同父变体系列）→ 变体跳转专门标记，不再按 asin_mismatch 失败
    assert saved["reason"] == "variant_redirect"
    assert saved["variant_asin"] == "B0B9ZFZZZZ"
    assert saved["terminal"] is False
    assert saved["evidence"]["error_code"] == "variant_redirect"
...
def test_empty_review_failure_preserves_review_stage_and_cursor():
    # 评论空页不拖死商品批次：商品已落库，成员按成功收尾；
    # 评论缺失记在 last_error + 评论汇总 status=failed（评论侧状态保留）
    worker = load_worker()
    storage = PortalReviewStorage()
    ...
    payload = storage.saved[0]
    assert payload["next_status"] == "succeeded"
    assert payload["state_fields"]["task_stage"] == "complete"
    assert payload["state_fields"]["next_review_url"] is None
    assert payload["state_fields"]["last_error"] == "empty_review_page"
    assert payload["summary"]["status"] == "failed"
    assert payload["summary"]["next_page"] is None
    assert payload["increment_attempts"] is False


def test_review_login_wall_finishes_member_as_succeeded():
    # 评论登录墙（亚马逊限制未登录浏览评论）不拖死商品批次：
    # 商品数据已落库，成员按成功收尾，last_error 记 login_wall
    worker = load_worker()
    storage = ReviewStorage()
    adapter = LoginWallReviewAdapter()
    config = dict(worker.DEFAULTS)
    config.update({"max_actions_per_run": 1, "raw_html_dir": None, "context": {}, "review_page_limit": 0})
    ...
    assert payload["state_fields"]["task_stage"] == "complete"
    assert payload["state_fields"]["next_review_url"] is None
    assert payload["state_fields"]["last_error"] == "login_wall"
    assert payload["summary"]["status"] == "login_wall"
    assert payload["reason"] == "review_login_wall"
    assert payload["increment_attempts"] is False
...
def test_postgres_runner_postal_mismatch_no_longer_retries_in_browser():
    # postal mismatch 是软错误，不再触发 browser fallback
    # HTTP 直接保存产品，context_quality 为 partial
    worker = load_worker()
    storage = Storage()
    ...
    assert worker.run_postgres_actions(storage, adapter, config, limit=1, worker_id="worker-a") == 1
    # 只有 HTTP，不触发 browser（postal mismatch 不是硬错误）
    assert adapter.calls == ["http"]
    # 产品仍然成功保存
    assert storage.saved[0]["next_status"] == "succeeded"
