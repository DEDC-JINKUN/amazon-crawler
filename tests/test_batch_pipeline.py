...
class BatchCheckpointTests(unittest.TestCase):
    ...
    def test_geo_mismatch_no_longer_retries_in_browser(self):
        # 货币/配送地址 mismatch 现在是软错误，不再触发 browser fallback
        # HTTP 路径直接保存产品（context_quality 可能是 partial，但不影响保存）
        ...
        class Adapter:
            ...
            def fetch_browser(self, url):
                # 不应该被调用——geo mismatch 不触发 browser
                self.calls.append(("browser", url))
                raise AssertionError("geo mismatch 不应触发 browser")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            # 价格保持 HTTP 原始值（HKD），没有 browser 修正
            self.assertEqual(snapshot[0], "HKD19.99")
            # 只有 HTTP 调用，不触发 browser
            self.assertEqual([kind for kind, _ in adapter.calls], ["http"])
            conn.close()
